...
function Profile() {
  ...
  async function handleAddPgxResult(event) {
    ...
    setPgxFormStatus({ status: "loading", message: "" });
    ...
    try {
      ...
      setPgxForm({ gene: "CYP2C19", phenotype: "poor metabolizer", genotype: "" });
      setPgxFormStatus({ status: "idle", message: "" });
      await loadPgxProfile();
      setActivePanel(null);
    } catch (error) {
      setPgxFormStatus({ status: "error", message: error.message });
    }
  }
  ...
  function handleSelectResultToEdit(result) {
    ...
    setEditForm({ gene: result.gene, phenotype, genotype: result.genotype || "" });
    setEditFormStatus({ status: "idle", message: "" });
  }
  ...
  async function handleUpdatePgxResult(event) {
    ...
    setEditFormStatus({ status: "loading", message: "" });
    ...
    try {
      ...
      await loadPgxProfile();
      setEditForm({ gene: "", phenotype: "", genotype: "" });
      setEditFormStatus({ status: "idle", message: "" });
      setActivePanel(null);
    } catch (error) {
      setEditFormStatus({ status: "error", message: error.message });
    }
  }
        ...
        <div className="dashboard-card">
          <div className="dashboard-card-header">
            <h2 className="dashboard-welcome">
              {user?.firstName || "Patient"}&apos;s PGx Profile
            </h2>
                ...
                <>
                  ...
                  {editForm.gene ? (
                    <form onSubmit={handleUpdatePgxResult}>
                      ...
                      <label className="search-label" htmlFor="edit-pgx-phenotype">
                        Phenotype
                      </label>
                      <select
                      ...
                      <label className="search-label" htmlFor="edit-pgx-genotype">
                        Genotype (optional)
                      </label>
                      <input
              ...
              <div className="pgx-results-list">
                {pgxState.results.map((result) => (
                  <div className="pgx-result-card" key={result.id}>
                    <h3 className="panel-heading">{result.gene}</h3>
                    <dl className="pgx-info-grid">
                      <dt>Phenotype</dt>
                      <dt>Genotype</dt>
